...
const noticeFetch = async (req, res) => {
    try {
        console.log("noticeFetch");
        const news = await publicService.noticefetch();
        res.json(news);
    } catch (error) {
        res.status(404).json({ error: "Some Error occured", message: error.message });
    }
};
const tenderFetch = async (req, res) => {
    try {
        console.log("TenderFetch");
        const news = await publicService.tenderfetch();
        res.json(news);
    } catch (error) {
        res.status(404).json({ error: "Some Error occured", message: error.message });
    }
...
module.exports = { getallFacultyList, getFacultyProfile, postfetchSlides , postAllDepartmentFaculty , announcementsFetch, newsFetch,noticeFetch,tenderFetch };
